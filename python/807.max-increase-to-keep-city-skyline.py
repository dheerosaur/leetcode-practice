...
class Solution:
    def maxIncreaseKeepingSkyline(self, grid: List[List[int]]) -> int:
        rm = [max(x) for x in grid]
        cm = [max(x) for x in zip(*grid)]

        return sum(min(rm[i], cm[j]) - val
                   for i, row in enumerate(grid)
                   for j, val in enumerate(row))

    def maxIncreaseKeepingSkyline_2(self, grid: List[List[int]]) -> int:
        total = 0
        td_maxes = [max(row) for row in grid]
        lr_maxes = [max(row[i] for row in grid) for i, _ in enumerate(grid[0])]
        for i, row in enumerate(grid):
            for j, val in enumerate(row):
                total += min(td_maxes[i], lr_maxes[j]) - val
        return total
...
def test():
    ...
    for case in cases:
        print(sol.maxIncreaseKeepingSkyline(case))
